...
struct node *Search(int skey, struct node *p, int *pn)// skey = search key
{
...
int search_node(int skey, struct node *p, int *pn)
{
if( skey < p->key[1] )//key doesnt exist as key is smaller than the smallest key then return 0
{
*pn = 0;//pointer remains at 0
return 0;
}
*pn = p->count;// what does PN do ?
while( ( skey < p->key[*pn] ) && *pn>1 )
(*pn)--; // reducing the pointer value till it reaches 0 so as to scan each key and match to skey
if( skey == p->key[*pn] )
return 1; // key found
else
return 0;
...
void delByShift( struct node *p, int n)// n is the number to be deleted
{
int i;
for( i = n+1; i <= p->count; i++ )// starting node is just after the desired node to be deleted
{
p->key[i-1] = p->key[i];
p->child[i-1] = p->child[i];
}
